<?php
     ...
     function genere_mat_consultation($size, $bdd) //, $bdd
     {
		...
		$verif = $bdd -> query("SELECT mat_consultation FROM consultation WHERE mat_consultation = '$mat_consultation'");
		//Si on ne trouve aucune ligne correspondant au matricule généré on retourne le mot de passe généré
		if($verif -> rowCount() == 0)
			return $mat_consultation;

		//Sinon on relance la fonction : RECURSIVITE !!!
		else 
			return genere_mat_consultation($size, $bdd);
